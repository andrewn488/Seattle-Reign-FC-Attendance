...
q2_vis <- q2_vis_wrangle %>% 
  ...
  labs(title = "Influences of satisfaction levels of Reign FC",
...
q6_viz <- dash_df %>% 
  ...
  group_by(Seahawks, Sounders, Mariners) %>% 
  summarize(n = mean(n())) %>% 
  gather(Questions, Response, -c(n))%>% 
  mutate(Response = as.factor(Response)) %>% 
  ...
  labs(title = "Popularity of Seattle teams among Reign FC fans",
       subtitle = "Preference to soccer over other professional Seattle sport")
